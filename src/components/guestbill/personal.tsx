...
import BillPersonal from './addPersonal'

function PersonalBill() {
  ...
  const [selectedEdit, setselectedEdit] = useState<number>(); 
    ...
    return (
      <div className="overflow-x-hidden  w-full min-h-screen"> 
        <BillPersonal  id={selectedEdit} isVisible={showModal2} onClose={()=>setShowModal2(false)}/>
                                 ...
                                 <tr key={item.id} className={`h-[60px] justify-items-center ${i % 2 === 0 ? 'bg-white' : 'bg-[#84D2D89C]'}`}>
                                    ...
                                    <td className='text-[18px] text-center px-4 py-2 '>{formatTanggal(item.checkin)}</td>
                                     ...
                                     <td className="px-4 py-2 flex justify-self-center">
                                        ...
                                        {/* <button onClick={()=> handleEditBill(item.id)}>edit</button>  */}
