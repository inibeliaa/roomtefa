...
//     setRows(updated);
  ...
  `).join('');
      ...
      const handleGuest = (id: number) => {
                            //   const data = data?.find((item) => item.id === id);
                              if (data) {
                                ...
                                // setService(serfis);
                                document.title = `${data.id}_Guest_Bill_${data.fullname}`;
                                const printContent = `
                                  <html>
                                    <head>
                                    ...
                                    <h2 class="text-center">GUEST BILL</h2>
                                      ...
                                      <div class="details flex">
                                        <p style="width: 60%">Guest Name: ${data?.fullname}</p>
                                        <div>
                                        <p>Room:   ${data?.roomRs?.map((item:any) => item.room).join(',')}</p>
                                        <p>Arrival: ${formatTanggal(data?.checkin)}</p>
                                        <p>Departure: ${formatTanggal(data?.checkout)}</p>
    ...
    useEffect(() => {
        const target = document.getElementById("main-content"); // ganti sesuai layout kamu
        if (!target) return;
  ...
  return (
    <div id='main-content' className="fixed inset-0 z-50 h-screen flex py-10 bg-[#5A72A0]/50 backdrop-blur-sm">
<div className="w-[70%] bg-[#84D2D89C] ms-11 max-h-[90vh] px-[3%] py-[3%] border-2 border-[#5A72A0] rounded-md flex flex-col overflow-hidden">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md overflow-y-auto max-h-[calc(100vh-150px)]">
            <table className="table-auto w-full border border-gray-300">
                ...
                <tr>
                    <td className="border px-4 py-2"></td>
                                  <td className="border px-4 py-2">Government Tax 
                                      (11%)
                    </td>
                    <td className="border px-4 py-2">
                    {formatHarga(tax)}
            ...
            <tr key={i}>
              <td className="border px-2 py-1">
                {/* <input
                  type="date"
                  className="text-center input text-center input-bordered w-full"
                //   value={row.date}
