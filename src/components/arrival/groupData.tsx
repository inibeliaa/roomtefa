...
function GroupArrival() {
    ...
    const filteredData = data.filter(item =>
        (item?.arrivalReservasi[0]?.datee?.toLowerCase() || "").includes(search.toLowerCase())
        // ||
                    ...
                    {filteredData.length > 0 ? (
                            filteredData.map((item: any, i: number) => (   
                                 <tr key={item.id} className={`h-[60px] justify-items-center ${i % 2 === 0 ? 'bg-white' : 'bg-[#84D2D89C]'}`}>
                                     ...
                                     */}
                                    <td className='text-[18px] text-center px-4 py-2 '>{formatTanggal(item.arrivalReservasi[0]?.datee)}</td>
                                     <td className='text-[18px] text-center px-4 py-2'>{(item.address)}</td>
                                    <td className="px-4 py-2 text-center"> {item && item.remarksRs?.map((remarks: any) => remarks.detail || '').join(',')}
                                    </td>
                                </tr>
                            ))
                        ) : (
