...
function PersonalDeparture() {
                ...
                <table className="w-full table-auto ">
                    <thead>
                        <tr className="bg-[#0E7793] h-[70px] text-white">
                            ...
                            <th className="px-4 py-2" rowSpan={2}>Departure Date</th>
 ...
 <td className='text-[18px] text-center px-4 py-2 whitespace-nowrap'> {item.roomRs && item.roomRs?.map((rooms: any) => rooms.room || '').join(',')}</td>
  ...
  {item.regisP && item.regisP.map((remarks: any) => remarks.detail || '').join(',')}
                                    </td>
                                </tr>
                            ))
                        ) : (
...
export default PersonalDeparture
