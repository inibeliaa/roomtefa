...
function GroupDeparture() {
    ...
    async function getIn() {
        const url = `${process.env.NEXT_PUBLIC_URL}api/regisG`;
        try {
    ...
    const filteredData = data.filter(item =>
        // (item.arrivalGroups[0]?.datee?.toLowerCase() || "").includes(search.toLowerCase()) ||
    ...
    return (
        <div className=" w-[1150px] min-h-screen">   
                    ...
                    {filteredData.length > 0 ? (
                            filteredData.map((item: any, i: number) => (   
                                 <tr key={item.id} className={`h-[60px] justify-items-center ${i % 2 === 0 ? 'bg-white' : 'bg-[#84D2D89C]'}`}>
                                    ...
                                    <td className='text-[18px] text-center px-4 py-2 '>{formatTanggal(item.arrivalGroups[0]?.datee)}</td>
                                     <td className='text-[18px] text-center px-4 py-2'>{formatTanggal(item.DepartureGroups[0]?.datee)}</td>
                                    <td className="px-4 py-2 text-center">
  {item.remarks && item.remarks?.map((remarks: any) => remarks.detail || '').join(',')}
                                    </td>
                                </tr>
                            ))
                        ) : (
                   ...
                   <table className="w-full table-auto">
                    <thead>
                        <tr className="bg-[#0E7793] h-[70px] text-white">
                            ...
                            <th className="px-4 py-2" rowSpan={2}>Name of Group</th>
    ...
    {/* <th className="px-4 py-2" colSpan={5}></th> */}
                    ...
                    {filteredData.length > 0 ? (
                            filteredData.map((item: any, i: number) => (   
                                 <tr key={item.id} className={`h-[60px] justify-items-center ${i % 2 === 0 ? 'bg-white' : 'bg-[#84D2D89C]'}`}>
                                    <td className='text-[18px] text-center px-4 py-2 whitespace-nowrap'>{item.id}</td>
                                    <td className='text-[18px] text-center px-4 py-2 whitespace-nowrap'> {item.roomRs && item.roomRs?.map((rooms: any) => rooms.room || '').join(',')}</td>
                                    <td className='text-[18px] text-center px-4 py-2 whitespace-nowrap'>{item.name}</td>
                                    ...
                                    <td className='text-[18px] text-center px-4 py-2 whitespace-nowrap'>{formatHarga(item.roomRs && item.roomRs?.map((rooms: any) => rooms.rate || '').join(','))}</td>
                                    ...
                                    <td className="px-4 py-2 text-center"> {item && item.regis?.map((remarks: any) => remarks.detail || '').join(',')}
                                    </td>
                                </tr>
                            ))
                        ) : (
