...
function Page() {
  ...
  const [wakeUp, setWakeUp] = useState("");
    ...
    async function Checkin() {
        // setIsLoading(true);
        const url = `${process.env.NEXT_PUBLIC_URL}api/checkin` 
        try {
                ...
                <ul className='flex-col space-y-[2%]'>
                  ...
                  <div className="flex space-x-5 w-full">
                    <div className='w-[50%]'>
                      ...
                      <input type="date" readOnly value={formatTanggalForInput(data.checkin)} name='Check In' className='bg-white p-2 w-full h-[50px] rounded-md' />
                    </div>
                    <div className='w-[50%]'>
                    <label htmlFor="Check Out" className="text-[20px]">Check Out</label>
                    <input type="date" name='Check Out' readOnly value={formatTanggalForInput(data.checkout)} className='bg-white p-2 w-full h-[50px] rounded-md' />
                  </div>  
            </div> 
            </>
          )}
